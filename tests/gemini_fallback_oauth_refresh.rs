...
//!
//! Requires:
//! - Live Gemini OAuth profile in `~/.octoclaw/auth-profiles.json` with refresh_token
...
async fn gemini_warmup_refreshes_expired_oauth_token() -> Result<()> {
    // Find ~/.octoclaw/auth-profiles.json
    let home = env::var("HOME").expect("HOME env var not set");
    let octoclaw_dir = PathBuf::from(home).join(".octoclaw");
    let auth_profiles_path = octoclaw_dir.join("auth-profiles.json");
    ...
    if !auth_profiles_path.exists() {
        ...
        eprintln!("Run: octoclaw auth login --provider gemini");
        return Ok(());
    }
    ...
    let gemini_profile_key = profiles
        ...
        .ok_or_else(|| {
            anyhow::anyhow!(
                "No Gemini OAuth profile found. Run: octoclaw auth login --provider gemini"
            )
        })?
        .clone();
    ...
    // This will load auth from ~/.octoclaw/auth-profiles.json (with expired token)
    let provider = octoclaw::providers::create_provider("gemini", None)?;
