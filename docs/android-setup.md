...
OctoClaw provides prebuilt binaries for Android devices.
...
The easiest way to run OctoClaw on Android is via [Termux](https://termux.dev/).
...
# For 64-bit (aarch64):
curl -LO https://github.com/octoclaw-labs/octoclaw/releases/latest/download/octoclaw-aarch64-linux-android.tar.gz
tar xzf octoclaw-aarch64-linux-android.tar.gz

# For 32-bit (armv7):
curl -LO https://github.com/octoclaw-labs/octoclaw/releases/latest/download/octoclaw-armv7-linux-androideabi.tar.gz
tar xzf octoclaw-armv7-linux-androideabi.tar.gz
```
...
For advanced users who want to run OctoClaw outside Termux:

```bash
# From your computer with ADB
adb push octoclaw /data/local/tmp/
adb shell chmod +x /data/local/tmp/octoclaw
adb shell /data/local/tmp/octoclaw --version
```
...
OctoClaw supports two Android source-build workflows.
...
scripts/android/termux_source_build_check.sh \
  ...
  --mode ndk-cross \
  --diagnose-log /path/to/cargo-error.log \
  --json-output /tmp/octoclaw-android-selfcheck.json
```
