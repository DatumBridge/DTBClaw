...
OctoClaw supports native integration with Mattermost via its REST API v4. This integration is ideal for self-hosted, private, or air-gapped environments where sovereign communication is a requirement.
...
2.  **Bot Account**:
    ...
    - Set a username (e.g., `octoclaw-bot`).
...
| `mention_only` | (Optional) When `true`, only messages that explicitly mention the bot username (for example `@octoclaw-bot`) are processed. Default: `false`. |

## Threaded Conversations

OctoClaw supports Mattermost threads in both modes:
- If a user sends a message in an existing thread, OctoClaw always replies within that same thread.
...
When `mention_only = true`, OctoClaw applies an extra filter after `allowed_users` authorization:
