...
This document is the canonical reference for channel configuration in OctoClaw.
...
6. **Stale runtime state**: config changed but `octoclaw daemon` was not restarted.

---

## 1. Configuration Namespace

All channel settings live under `channels_config` in `~/.octoclaw/config.toml`.
...
One OctoClaw runtime can serve multiple channels at once: if you configure several
channel sub-tables, `octoclaw channel start` launches all of them in the same process.
...
When running `octoclaw channel start` (or daemon mode), runtime commands include:
...
- Model cache previews come from `octoclaw models refresh --provider <ID>`.
...
OctoClaw supports multimodal input through inline message markers:
...
If `[channels_config.matrix]`, `[channels_config.lark]`, or `[channels_config.feishu]` is present but the corresponding feature is not compiled in, `octoclaw channel list`, `octoclaw channel doctor`, and `octoclaw channel start` will report that the channel is intentionally skipped for this build. The same applies to cron delivery: setting `delivery.channel` to a feature-gated channel in a build without that feature will return an error at delivery time. For Matrix cron delivery, only plain rooms are supported; E2EE rooms require listener sessions via `octoclaw daemon`.
...
access_token = "syt_..."
user_id = "@octoclaw:matrix.example.com"   # optional, recommended for E2EE
...
OctoClaw supports two WhatsApp backends:
...
```toml
[channels_config.whatsapp]
session_path = "~/.octoclaw/state/whatsapp-web/session.db"
...
allowed_senders = ["*"]
imap_id = { enabled = true, name = "octoclaw", version = "0.1.7", vendor = "octoclaw-labs" }
```
...
port = 6697
nickname = "octoclaw-bot"
username = "octoclaw"              # optional
channels = ["#octoclaw"]
allowed_users = ["*"]
...
- If image download fails, OctoClaw forwards fallback text instead of silently dropping the message.
...
- `OCTOCLAW_NEXTCLOUD_TALK_WEBHOOK_SECRET` overrides config secret.
...
- `OCTOCLAW_LINQ_SIGNING_SECRET` overrides config secret.
...
- `OCTOCLAW_WATI_WEBHOOK_SECRET` overrides `webhook_secret` when set.

### 4.21 ACP

ACP (Agent Client Protocol) enables OctoClaw to act as a client for OpenCode ACP server,
...
5. Restart `octoclaw daemon` after config changes.
...
```bash
RUST_LOG=info octoclaw daemon 2>&1 | tee /tmp/octoclaw.log
```

Then filter channel/gateway events:

```bash
rg -n "Matrix|Telegram|Discord|Slack|Mattermost|Signal|WhatsApp|Email|IRC|Lark|DingTalk|QQ|iMessage|Nostr|Webhook|Channel|ACP" /tmp/octoclaw.log
```
