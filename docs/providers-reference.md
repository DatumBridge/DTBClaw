...
3. Generic fallback env vars: `OCTOCLAW_API_KEY` then `API_KEY`
...
- **Quota Tracking**: `octoclaw providers-quota --provider qwen-code` shows static quota info (`?/1000` - unknown remaining, 1000/day total)
...
octoclaw onboard --provider volcengine --api-key "$ARK_API_KEY" --model doubao-1-5-pro-32k-250115 --force
```

Quick validation:

```bash
octoclaw models refresh --provider volcengine
octoclaw agent --provider volcengine --model doubao-1-5-pro-32k-250115 -m "ping"
```
...
octoclaw onboard --provider stepfun --api-key "$STEP_API_KEY" --model step-3.5-flash --force
```
...
octoclaw onboard --provider siliconflow --api-key "$SILICONFLOW_API_KEY" --model Pro/zai-org/GLM-4.7 --force
```

Quick validation:

```bash
octoclaw models refresh --provider siliconflow
octoclaw agent --provider siliconflow --model Pro/zai-org/GLM-4.7 -m "ping"
```
...
- After multimodal normalization, OctoClaw sends image payloads through Ollama's native `messages[].images` field.
- If a non-vision provider is selected, OctoClaw returns a structured capability error instead of silently ignoring images.
...
- OctoClaw normalizes a trailing `/api` in `api_url` automatically.
...
- Model discovery: `octoclaw models refresh --provider llamacpp`
...
- Model discovery: `octoclaw models refresh --provider sglang`
...
- Model discovery: `octoclaw models refresh --provider vllm`
...
- Model discovery: `octoclaw models refresh --provider osaurus`
...
Since OctoClaw cannot auto-detect this, you can override it in `config.toml`:
...
Environment override: `OCTOCLAW_MODEL_SUPPORT_VISION=true`
...
- When set, overrides `OCTOCLAW_CODEX_REASONING_EFFORT`.
...
- Model discovery: `octoclaw models refresh --provider nvidia`
