...
Issue: [#2263](https://github.com/octoclaw-labs/octoclaw/issues/2263)
...
```bash
scripts/ci/m4_5_rfi_baseline.sh /tmp/octoclaw-m4rfi-target
```
...
1. `crates/octoclaw-types`
- shared DTOs, enums, IDs, lightweight cross-domain traits
- no provider/channel/network dependencies

1. `crates/octoclaw-core`
- config structs + validation, provider trait contracts, routing primitives, policy helpers
- depends on `octoclaw-types`

1. `crates/octoclaw-memory`
- memory traits/backends + hygiene/snapshot plumbing
- depends on `octoclaw-types`, `octoclaw-core` contracts only where required

1. `crates/octoclaw-channels`
- channel adapters + inbound normalization
- depends on `octoclaw-types`, `octoclaw-core`, `octoclaw-memory`

1. `crates/octoclaw-api`
- gateway/webhook/http orchestration
- depends on `octoclaw-types`, `octoclaw-core`, `octoclaw-memory`, `octoclaw-channels`

1. `crates/octoclaw-bin` (or keep root binary package name stable)
...
- keep provider-specific SDK deps out of `octoclaw-types`
...
- add package-focused lanes for extracted crates (`-p octoclaw-types`, `-p octoclaw-core`, etc.)
...
- extract low-churn shared types into `octoclaw-types`
- add re-export shim layer to preserve existing import paths

### PR-3 (S)

- extract config/provider contracts into `octoclaw-core`
