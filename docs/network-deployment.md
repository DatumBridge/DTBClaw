# Network Deployment — OctoClaw on Raspberry Pi and Local Network

This document covers deploying OctoClaw on a Raspberry Pi or other host on your local network, with Telegram and optional webhook channels.
...
|------|----------------------|----------|
| **Telegram polling** | No | OctoClaw polls Telegram API; works from anywhere |
| **Matrix sync (including E2EE)** | No | OctoClaw syncs via Matrix client API; no inbound webhook required |
...
**Key:** Telegram, Discord, Slack, and Nostr use **outbound connections** — OctoClaw connects to external servers/relays. No port forwarding or public IP required.

---

## 2. OctoClaw on Raspberry Pi
...
- Telegram channel works: OctoClaw polls Telegram API (outbound)
...
```bash
octoclaw daemon --host 0.0.0.0 --port 42617
```
...
2. Start a tunnel:
   ...
   ```
   Or use `octoclaw tunnel` (see tunnel docs).

3. OctoClaw will refuse `0.0.0.0` unless `allow_public_bind = true` or a tunnel is active.
...
- OctoClaw calls `https://api.telegram.org/bot{token}/getUpdates`
...
Run `octoclaw daemon` — Telegram channel starts automatically.
...
- Keep one runtime instance for the same token (recommended: `octoclaw daemon` service).
...
- [ ] Run `octoclaw daemon --host 127.0.0.1 --port 42617` (Telegram works without 0.0.0.0)
...
OctoClaw supports OpenRC for Alpine Linux and other distributions using the OpenRC init system. OpenRC services run **system-wide** and require root/sudo.

### 7.1 Prerequisites

- Alpine Linux (or another OpenRC-based distro)
- Root or sudo access
- A dedicated `octoclaw` system user (created during install)

### 7.2 Install Service

```bash
# Install service (OpenRC is auto-detected on Alpine)
sudo octoclaw service install
```

This creates:
- Init script: `/etc/init.d/octoclaw`
- Config directory: `/etc/octoclaw/`
- Log directory: `/var/log/octoclaw/`

### 7.3 Configuration

Manual config copy is usually not required.

`sudo octoclaw service install` automatically prepares `/etc/octoclaw`, migrates existing runtime state from your user setup when available, and sets ownership/permissions for the `octoclaw` service user.

If no prior runtime state is available to migrate, create `/etc/octoclaw/config.toml` before starting the service.

### 7.4 Enable and Start

```bash
# Add to default runlevel
sudo rc-update add octoclaw default

# Start the service
sudo rc-service octoclaw start

# Check status
sudo rc-service octoclaw status
```
...
| Command | Description |
|---------|-------------|
| `sudo rc-service octoclaw start` | Start the daemon |
| `sudo rc-service octoclaw stop` | Stop the daemon |
| `sudo rc-service octoclaw status` | Check service status |
| `sudo rc-service octoclaw restart` | Restart the daemon |
| `sudo octoclaw service status` | OctoClaw status wrapper (uses `/etc/octoclaw` config) |
...
| Log | Path |
|-----|------|
| Access/stdout | `/var/log/octoclaw/access.log` |
| Errors/stderr | `/var/log/octoclaw/error.log` |

View logs:

```bash
sudo tail -f /var/log/octoclaw/error.log
```
...
sudo rc-service octoclaw stop
sudo rc-update del octoclaw default

# Remove init script
sudo octoclaw service uninstall
```
...
- The service runs as the `octoclaw:octoclaw` user (least privilege)
- Config must be at `/etc/octoclaw/config.toml` (explicit path in init script)
- If the `octoclaw` user does not exist, install will fail with instructions to create it

### 7.9 Checklist: Alpine/OpenRC Deployment

- [ ] Install: `sudo octoclaw service install`
- [ ] Enable: `sudo rc-update add octoclaw default`
- [ ] Start: `sudo rc-service octoclaw start`
- [ ] Verify: `sudo rc-service octoclaw status`
- [ ] Check logs: `/var/log/octoclaw/error.log`
