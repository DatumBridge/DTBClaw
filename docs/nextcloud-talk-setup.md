...
This guide covers native Nextcloud Talk integration for OctoClaw.
...
Add this section in `~/.octoclaw/config.toml`:
...
octoclaw daemon
# or
octoclaw gateway --host 127.0.0.1 --port 3000
```
...
When `webhook_secret` is configured, OctoClaw verifies:
...
- OctoClaw ignores bot-originated webhook events (`actorType = bots`).
- OctoClaw ignores non-message/system events.
...
3. Confirm OctoClaw receives and replies in the same room.
