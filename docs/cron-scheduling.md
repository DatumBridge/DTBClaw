...
OctoClaw includes a full-featured job scheduling system for running tasks on a schedule, at specific times, or at regular intervals.

## Quick Start

```bash
# Add a cron job (runs every day at 9 AM)
octoclaw cron add '0 9 * * *' 'echo "Good morning!"'

# Add a one-shot reminder (runs in 30 minutes)
octoclaw cron once 30m 'notify-send "Time is up!"'
...
octoclaw cron add '0 9 * * 1-5' --tz 'America/Los_Angeles' 'echo "Work time"'

# Every hour
octoclaw cron add '0 * * * *' 'echo "Hourly check"'
...
octoclaw cron add-at '2026-03-15T14:30:00Z' 'echo "Meeting starts!"'

# Relative delay (human-friendly)
octoclaw cron once 2h 'echo "Two hours later"'
octoclaw cron once 30m 'echo "Half hour reminder"'
octoclaw cron once 1d 'echo "Tomorrow"'
```
...
```bash
octoclaw cron add '0 6 * * *' 'backup.sh && notify-send "Backup done"'
```
...
| Command | Description |
|---------|-------------|
| `octoclaw cron list` | Show all scheduled jobs |
| `octoclaw cron add <expr> <cmd>` | Add cron-expression job |
| `octoclaw cron add-at <time> <cmd>` | Add one-shot at time |
| `octoclaw cron add-every <ms> <cmd>` | Add interval job |
| `octoclaw cron once <delay> <cmd>` | Add one-shot with delay |
| `octoclaw cron update <id> [opts]` | Update job settings |
| `octoclaw cron remove <id>` | Delete a job |
| `octoclaw cron pause <id>` | Pause (disable) job |
| `octoclaw cron resume <id>` | Resume (enable) job |

## Configuration File

Define jobs in `octoclaw.toml`:
...
OctoClaw's cron system is compatible with OpenClaw's scheduling:
...
**Key difference:** OctoClaw uses TOML config format, OpenClaw uses JSON.
...
**Job not running?**
- Check `octoclaw cron list` - is it enabled?
