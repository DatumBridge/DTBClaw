...
| Scenario | Command |
|----------|---------|
| I have an API key, want fastest setup | `octoclaw onboard --api-key sk-... --provider openrouter` |
| I want guided prompts | `octoclaw onboard --interactive` |
| Config exists, just fix channels | `octoclaw onboard --channels-only` |
| Config exists, I intentionally want full overwrite | `octoclaw onboard --force` |
| Using OpenAI Codex subscription auth | See [OpenAI Codex OAuth Quick Setup](#openai-codex-oauth-quick-setup) |

## Onboarding and Validation

- Quick onboarding: `octoclaw onboard --api-key "sk-..." --provider openrouter`
- Interactive onboarding: `octoclaw onboard --interactive`
...
- Validate environment: `octoclaw status` + `octoclaw doctor`
...
```bash
octoclaw auth login --provider openai-codex
```

2. Verify auth material is loaded:

```bash
octoclaw auth status --provider openai-codex
```
