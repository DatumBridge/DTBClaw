...
This page defines the fastest supported path to install and initialize OctoClaw.
...
Default behavior is **app-only** (build/install OctoClaw) and expects existing Rust toolchain.
...
```bash
curl -fsSL https://raw.githubusercontent.com/octoclaw-labs/octoclaw/main/scripts/install.sh | bash
```
...
This builds a local OctoClaw image and launches onboarding inside a container while
persisting config/workspace to `./.octoclaw-docker`.

Container CLI defaults to `docker`. If Docker CLI is unavailable and `podman` exists,
bootstrap auto-falls back to `podman`. You can also set `OCTOCLAW_CONTAINER_CLI`
explicitly (for example: `OCTOCLAW_CONTAINER_CLI=podman ./bootstrap.sh --docker`).
...
Docker tag (`OCTOCLAW_DOCKER_IMAGE`, default: `octoclaw-bootstrap:local`); if missing,
it pulls `ghcr.io/octoclaw-labs/octoclaw:latest` and tags it locally before running.
...
```bash
OCTOCLAW_API_KEY="sk-..." OCTOCLAW_PROVIDER="openrouter" ./bootstrap.sh --onboard
```
...
- `--install-system-deps`
- `--install-rust`
- `--skip-build` (in `--docker` mode: use local image if present, otherwise pull `ghcr.io/octoclaw-labs/octoclaw:latest`)
- `--skip-install`
- `--provider <id>`
