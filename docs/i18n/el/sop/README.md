...
- Το CLI `octoclaw sop` διαχειρίζεται αυτή τη στιγμή μόνο ορισμούς: `list`, `validate`, `show`.
...
2. Δημιουργήστε έναν κατάλογο SOP, για παράδειγμα:
   ...
   ```text
   ~/.octoclaw/workspace/sops/deploy-prod/SOP.toml
   ~/.octoclaw/workspace/sops/deploy-prod/SOP.md
   ```
...
3. Επαλήθευση και επιθεώρηση ορισμών:
   ...
   ```bash
   octoclaw sop list
   octoclaw sop validate
   octoclaw sop show deploy-prod
   ```
