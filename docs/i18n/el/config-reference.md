# Οδηγός Ρυθμίσεων OctoClaw (config.toml)
...
Το OctoClaw ψάχνει για τις ρυθμίσεις με την εξής σειρά:
1. Στη διαδρομή που ορίζει η μεταβλητή `OCTOCLAW_WORKSPACE`.
2. Στο αρχείο `~/.octoclaw/config.toml` (αυτή είναι η συνηθισμένη θέση).
...
- Αν αλλάξετε το αρχείο `config.toml`, πρέπει να κάνετε επανεκκίνηση το OctoClaw για να δει τις αλλαγές.
- Χρησιμοποιήστε την εντολή `octoclaw doctor` για να βεβαιωθείτε ότι οι ρυθμίσεις σας είναι σωστές.
