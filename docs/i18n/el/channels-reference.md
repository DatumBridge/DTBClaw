...
Αυτός ο οδηγός περιγράφει τη διαδικασία διαμόρφωσης των καναλιών επικοινωνίας (Telegram, Discord, κ.λπ.) στο OctoClaw.
...
Οι ρυθμίσεις αποθηκεύονται στο αρχείο `~/.octoclaw/config.toml`, στην ενότητα `[channels_config]`.
...
Σε πλατφόρμες όπως το Telegram και το Discord, μπορείτε να ελέγχετε το OctoClaw με εντολές:
...
Το OctoClaw υποστηρίζει την ανάγνωση εικόνων μέσω της σύνταξης:
`[IMAGE:<source>]`
...
5. **Επανεκκίνηση**: Κάθε αλλαγή στο `config.toml` απαιτεί επανεκκίνηση του `octoclaw daemon`.

### Διαγνωστικά Μηνύματα (Logs)

Εκτελέστε το OctoClaw με αυξημένο επίπεδο καταγραφής:
```bash
