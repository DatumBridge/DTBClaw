...
OctoClaw bao gồm hệ thống lập lịch công việc đầy đủ tính năng để chạy các tác vụ theo lịch trình, tại thời điểm cụ thể, hoặc theo khoảng thời gian đều đặn.

## Bắt đầu nhanh

```bash
# Thêm cron job (chạy mỗi ngày lúc 9 giờ sáng)
octoclaw cron add '0 9 * * *' 'echo "Chào buổi sáng!"'

# Thêm nhắc nhở một lần (chạy sau 30 phút)
octoclaw cron once 30m 'notify-send "Hết giờ!"'

# Thêm job theo khoảng thời gian (chạy mỗi 5 phút)
octoclaw cron add-every 300000 'curl -s http://api.example.com/health'

# Liệt kê tất cả jobs
octoclaw cron list

# Xóa một job
octoclaw cron remove <job-id>
```
...
octoclaw cron add '0 9 * * 1-5' --tz 'America/Los_Angeles' 'echo "Giờ làm việc"'

# Mỗi giờ
octoclaw cron add '0 * * * *' 'echo "Kiểm tra hàng giờ"'

# Mỗi 15 phút
octoclaw cron add '*/15 * * * *' 'curl http://localhost:8080/ping'
```
...
octoclaw cron add-at '2026-03-15T14:30:00Z' 'echo "Cuộc họp bắt đầu!"'

# Độ trễ tương đối (thân thiện với người dùng)
octoclaw cron once 2h 'echo "Hai giờ sau"'
octoclaw cron once 30m 'echo "Nhắc nhở nửa giờ"'
octoclaw cron once 1d 'echo "Ngày mai"'
```
...
octoclaw cron add-every 300000 'echo "Ping"'

# Mỗi giờ (3600000 ms)
octoclaw cron add-every 3600000 'curl http://api.example.com/sync'
```
...
```bash
octoclaw cron add '0 6 * * *' 'backup.sh && notify-send "Sao lưu xong"'
```
...
| Lệnh | Mô tả |
|------|-------|
| `octoclaw cron list` | Hiển thị tất cả jobs đã lập lịch |
| `octoclaw cron add <expr> <cmd>` | Thêm job với biểu thức cron |
| `octoclaw cron add-at <time> <cmd>` | Thêm job chạy một lần tại thời điểm |
| `octoclaw cron add-every <ms> <cmd>` | Thêm job theo khoảng thời gian |
| `octoclaw cron once <delay> <cmd>` | Thêm job chạy một lần với độ trễ |
| `octoclaw cron update <id> [opts]` | Cập nhật cài đặt job |
| `octoclaw cron remove <id>` | Xóa một job |
| `octoclaw cron pause <id>` | Tạm dừng (vô hiệu hóa) job |
| `octoclaw cron resume <id>` | Tiếp tục (kích hoạt) job |

## Tệp cấu hình

Định nghĩa jobs trong `octoclaw.toml`:
...
Hệ thống cron của OctoClaw tương thích với lập lịch của OpenClaw:
...
**Khác biệt chính:** OctoClaw sử dụng định dạng TOML, OpenClaw sử dụng JSON.
...
**Job không chạy?**
- Kiểm tra `octoclaw cron list` - nó có được bật không?
