...
[INFO] ✓ Audit logging enabled (~/.config/octoclaw/audit.log)
...
$ octoclaw security --status
Security Status:
  ✓ Sandbox: Landlock (Linux kernel 6.2)
  ✓ Memory monitoring: 512MB limit
  ✓ Audit logging: ~/.config/octoclaw/audit.log
  → 47 events logged today

# Tắt sandbox tường minh (ghi vào config)
$ octoclaw config set security.sandbox.enabled false

# Bật backend cụ thể
$ octoclaw config set security.sandbox.backend firejail

# Điều chỉnh giới hạn
$ octoclaw config set security.resources.max_memory_mb 2048
```
...
```bash
$ octoclaw onboard --security-level paranoid
[1/9] Workspace Setup...
...
...
✅ **Hiển thị trạng thái** — `octoclaw security --status`
