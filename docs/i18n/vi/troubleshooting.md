...
- `cargo check` / `cargo build` dừng lâu ở `Checking octoclaw`
...
### Không tìm thấy lệnh `octoclaw` sau cài đặt

Triệu chứng:

- Cài đặt thành công nhưng shell không tìm thấy `octoclaw`
...
- Dừng các tiến trình `octoclaw daemon` / `octoclaw channel start` thừa
