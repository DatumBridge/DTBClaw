# Triển khai mạng — OctoClaw trên Raspberry Pi và mạng nội bộ

Tài liệu này hướng dẫn triển khai OctoClaw trên Raspberry Pi hoặc host khác trong mạng nội bộ, với các channel Telegram và webhook tùy chọn.
...
|------|----------------------|----------|
| **Telegram polling** | Không | OctoClaw poll Telegram API; hoạt động từ bất kỳ đâu |
| **Matrix sync (kể cả E2EE)** | Không | OctoClaw sync qua Matrix client API; không cần webhook đến |
...
**Lưu ý:** Telegram, Discord và Slack dùng **long-polling** — OctoClaw thực hiện các request ra ngoài. Không cần port forwarding hoặc public IP.

---

## 2. OctoClaw trên Raspberry Pi
...
Chỉnh sửa `~/.octoclaw/config.toml`:
...
- Channel Telegram hoạt động: OctoClaw poll Telegram API (outbound)
...
```bash
octoclaw daemon --host 0.0.0.0 --port 3000
```
...
2. Khởi động tunnel:
   ...
   ```
   Hoặc dùng `octoclaw tunnel` (xem tài liệu tunnel).

3. OctoClaw sẽ từ chối `0.0.0.0` trừ khi `allow_public_bind = true` hoặc có tunnel đang hoạt động.
...
- OctoClaw gọi `https://api.telegram.org/bot{token}/getUpdates`
...
Chạy `octoclaw daemon` — channel Telegram khởi động tự động.
...
- Chỉ chạy một instance runtime cho cùng token (khuyến nghị: service `octoclaw daemon`).
...
- [ ] Chạy `octoclaw daemon --host 127.0.0.1 --port 3000` (Telegram hoạt động không cần 0.0.0.0)
