# Thêm Board và Tool — Hướng dẫn phần cứng OctoClaw

Hướng dẫn này giải thích cách thêm board phần cứng mới và tool tùy chỉnh vào OctoClaw.

## Bắt đầu nhanh: Thêm board qua CLI

```bash
# Thêm board (cập nhật ~/.octoclaw/config.toml)
octoclaw peripheral add nucleo-f401re /dev/ttyACM0
octoclaw peripheral add arduino-uno /dev/cu.usbmodem12345
octoclaw peripheral add rpi-gpio native   # cho Raspberry Pi GPIO (Linux)
...
Với feature `rag-pdf`, OctoClaw có thể lập chỉ mục file PDF:
...
2. **Thêm vào config** — `octoclaw peripheral add my-board /dev/ttyUSB0`
...
| Lệnh | Mô tả |
|------|-------|
| `octoclaw peripheral list` | Liệt kê các board đã cấu hình |
| `octoclaw peripheral add <board> <path>` | Thêm board (ghi vào config) |
| `octoclaw peripheral flash` | Nạp firmware Arduino |
| `octoclaw peripheral flash-nucleo` | Nạp firmware Nucleo |
| `octoclaw hardware discover` | Liệt kê thiết bị USB |
| `octoclaw hardware info` | Thông tin chip qua probe-rs |
