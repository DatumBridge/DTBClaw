# OctoClaw on Nucleo-F401RE — Step-by-Step Guide

Run OctoClaw on your Mac or Linux host. Connect a Nucleo-F401RE via USB. Control GPIO (LED, pins) via Telegram or CLI.

---

## Get Board Info via Telegram (No Firmware Needed)

OctoClaw can read chip info from the Nucleo over USB **without flashing any firmware**. Message your Telegram bot:
...
OctoClaw includes everything for Nucleo-F401RE:
...
| Flash command | `octoclaw peripheral flash-nucleo` | Builds firmware, flashes via probe-rs |
...
### 1.2 Flash via OctoClaw

From the octoclaw repo root:

```bash
octoclaw peripheral flash-nucleo
```

This builds `firmware/octoclaw-nucleo` and runs `probe-rs run --chip STM32F401RETx`. The firmware runs immediately after flashing.
...
| 5 | `octoclaw daemon` or `octoclaw agent -m "Turn on LED"` |
...
- **GPIO commands ignored** — Check `path` in config matches your serial port. Run `octoclaw peripheral list` to verify.
