# Adding Boards and Tools — OctoClaw Hardware Guide

This guide explains how to add new hardware boards and custom tools to OctoClaw.

## Quick Start: Add a Board via CLI

```bash
# Add a board (updates ~/.octoclaw/config.toml)
octoclaw peripheral add nucleo-f401re /dev/ttyACM0
octoclaw peripheral add arduino-uno /dev/cu.usbmodem12345
octoclaw peripheral add rpi-gpio native   # for Raspberry Pi GPIO (Linux)
...
With the `rag-pdf` feature, OctoClaw can index PDF files:
...
2. **Add to config** — `octoclaw peripheral add my-board /dev/ttyUSB0`
...
| Command | Description |
|---------|-------------|
| `octoclaw peripheral list` | List configured boards |
| `octoclaw peripheral add <board> <path>` | Add board (writes config) |
| `octoclaw peripheral flash` | Flash Arduino firmware |
| `octoclaw peripheral flash-nucleo` | Flash Nucleo firmware |
| `octoclaw hardware discover` | List USB devices |
| `octoclaw hardware info` | Chip info via probe-rs |
