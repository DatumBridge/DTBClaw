...
OctoClaw supports custom API endpoints for both OpenAI-compatible and Anthropic-compatible providers.
...
- Auto mode: when your `custom:` endpoint resolves to `api.openai.com`, OctoClaw will try WebSocket mode first (`wss://.../responses`) and automatically fall back to HTTP if the websocket handshake or stream fails.
- Manual override:
  - `OCTOCLAW_RESPONSES_WEBSOCKET=1` forces websocket-first mode for any `custom:` endpoint.
  - `OCTOCLAW_RESPONSES_WEBSOCKET=0` disables websocket mode and uses HTTP only.
...
# or: export OCTOCLAW_API_KEY="your-api-key"
octoclaw agent
```

## Hunyuan (Tencent)

OctoClaw includes a first-class provider for [Tencent Hunyuan](https://hunyuan.tencent.com/):
...
OctoClaw includes a first-class local provider for `llama-server`:
...
```bash
octoclaw models refresh --provider llamacpp
octoclaw agent -m "hello"
```

You do not need to export `OCTOCLAW_API_KEY=dummy` for this flow.

## SGLang Server

OctoClaw includes a first-class local provider for [SGLang](https://github.com/sgl-project/sglang):
...
```bash
octoclaw models refresh --provider sglang
octoclaw agent -m "hello"
```
...
OctoClaw includes a first-class local provider for [vLLM](https://docs.vllm.ai/):
...
```bash
octoclaw models refresh --provider vllm
octoclaw agent -m "hello"
```
