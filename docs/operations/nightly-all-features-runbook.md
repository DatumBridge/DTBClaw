...
- `gh run list --repo octoclaw-labs/octoclaw --workflow feature-matrix.yml --limit 10`
- `gh run view <run_id> --repo octoclaw-labs/octoclaw --json jobs,headSha,event,createdAt,url`
- inspect `nightly-history.json` in `nightly-all-features-summary` artifact

Manual trigger (nightly profile):

- `gh workflow run feature-matrix.yml --repo octoclaw-labs/octoclaw --ref dev -f profile=nightly -f fail_on_failure=true`
