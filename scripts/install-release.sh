...
pick_install_dir() {
  if [ -n "${OCTOCLAW_INSTALL_DIR:-}" ]; then
    echo "$OCTOCLAW_INSTALL_DIR"
    return 0
  fi
...
Installs the latest Linux OctoClaw binary from official GitHub releases.

Options:
  --no-onboard   Install only; do not run `octoclaw onboard`

Environment:
  OCTOCLAW_INSTALL_DIR  Override install directory
EOF
      exit 0
      ;;
    *)
...
if [ ! -f "$TMP_DIR/octoclaw" ]; then
  echo "error: release archive does not contain expected 'octoclaw' binary" >&2
  exit 1
fi

INSTALL_DIR="$(pick_install_dir)"
BIN_PATH="${INSTALL_DIR}/octoclaw"

if [ "${INSTALL_DIR#/usr/local/}" != "$INSTALL_DIR" ]; then
  run_privileged mkdir -p "$INSTALL_DIR"
  run_privileged install -m 0755 "$TMP_DIR/octoclaw" "$BIN_PATH"
else
  mkdir -p "$INSTALL_DIR"
  install -m 0755 "$TMP_DIR/octoclaw" "$BIN_PATH"
fi

echo "==> Installed: $BIN_PATH"
if ! command -v octoclaw >/dev/null 2>&1; then
