...
SYSTEM_PROMPT = """You are OctoClaw, an AI assistant with tool access. Use tools to accomplish tasks.
...
class OctoclawAgent:
    ...
    def invoke(self, input: dict[str, Any], config: Optional[dict] = None) -> dict:
        ...
        raise RuntimeError(
            "OctoclawAgent.invoke() cannot be called inside an active event loop. "
            "Use 'await OctoclawAgent.ainvoke(...)' instead."
        )
...
def create_agent(
    ...
    """
    Create an OctoClaw agent with LangGraph-based tool calling.
    ...
    Returns:
        Configured OctoclawAgent instance
