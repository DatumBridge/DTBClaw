...
//! JSON to OctoClaw's system prompt format.
...
const IDENTITY_BACKENDS: [IdentityBackendProfile; 2] = [
    IdentityBackendProfile {
        ...
        description: "Classic OctoClaw layout with IDENTITY.md, SOUL.md, USER.md, and friends.",
    },
    IdentityBackendProfile {
        key: "aieos",
...
pub fn generate_default_aieos_json(agent_name: &str, user_name: &str) -> String {
    ...
    serde_json::json!({
        "identity": {
            ...
            "bio": format!(
                "{resolved_agent_name} is a OctoClaw assistant focused on helping {resolved_user_name} get work done efficiently."
            ),
            "origin": "OctoClaw",
            "residence": "Workspace"
        },
...
mod tests {
    ...
    fn test_workspace_dir() -> PathBuf {
        std::env::temp_dir().join("octoclaw-test-identity")
    }
