...
// in `~/.octoclaw/.secret_key` with restrictive file permissions (0600). The
...
pub struct SecretStore {
    /// Path to the key file (`~/.octoclaw/.secret_key`)
    key_path: PathBuf,
...
impl SecretStore {
    ...
    pub fn new(octoclaw_dir: &Path, enabled: bool) -> Self {
        Self {
            key_path: octoclaw_dir.join(".secret_key"),
            enabled,
        }
    }
