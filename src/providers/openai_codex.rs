...
const CODEX_RESPONSES_URL_ENV: &str = "OCTOCLAW_CODEX_RESPONSES_URL";
const CODEX_BASE_URL_ENV: &str = "OCTOCLAW_CODEX_BASE_URL";
const CODEX_TRANSPORT_ENV: &str = "OCTOCLAW_CODEX_TRANSPORT";
const CODEX_PROVIDER_TRANSPORT_ENV: &str = "OCTOCLAW_PROVIDER_TRANSPORT";
const CODEX_RESPONSES_WEBSOCKET_ENV_LEGACY: &str = "OCTOCLAW_RESPONSES_WEBSOCKET";
const DEFAULT_CODEX_INSTRUCTIONS: &str =
    "You are OctoClaw, a concise and helpful coding assistant.";
...
impl OpenAiCodexProvider {
    pub fn new(
        ...
        let state_dir = options
            .octoclaw_dir
            .clone()
            .unwrap_or_else(default_octoclaw_dir);
...
fn default_octoclaw_dir() -> PathBuf {
    directories::UserDirs::new().map_or_else(
        || PathBuf::from(".octoclaw"),
        |dirs| dirs.home_dir().join(".octoclaw"),
    )
}
...
fn resolve_reasoning_effort(model_id: &str, override_level: Option<&str>) -> String {
    ...
    let env_level = std::env::var("OCTOCLAW_CODEX_REASONING_EFFORT")
        .ok()
        .and_then(|value| {
            normalize_reasoning_level(Some(&value), "OCTOCLAW_CODEX_REASONING_EFFORT")
        });
...
impl OpenAiCodexProvider {
    ...
    fn apply_auth_headers_ws(
        ...
        headers.insert(USER_AGENT, WsHeaderValue::from_static("octoclaw"));
    ...
    async fn send_responses_request(
        ...
        let access_token = if use_gateway_api_key_auth {
            ...
            Some(oauth_access_token.ok_or_else(|| {
                anyhow::anyhow!(
                    "OpenAI Codex auth profile not found. Run `octoclaw auth login --provider openai-codex`."
                )
            })?)
        };
        let account_id = if use_gateway_api_key_auth {
            ...
            Some(account_id.ok_or_else(|| {
                anyhow::anyhow!(
                    "OpenAI Codex account id not found in auth profile/token. Run `octoclaw auth login --provider openai-codex` again."
                )
            })?)
        };
...
mod tests {
    ...
    fn resolve_transport_mode_defaults_to_auto() {
        ...
        let _provider_guard = EnvGuard::set("OCTOCLAW_PROVIDER_TRANSPORT", None);
    ...
    fn resolve_reasoning_effort_prefers_config_override() {
        let _env_lock = env_lock();
        let _reasoning_guard = EnvGuard::set("OCTOCLAW_CODEX_REASONING_EFFORT", Some("low"));
    ...
    fn resolve_reasoning_effort_falls_back_to_env_when_override_invalid() {
        let _env_lock = env_lock();
        let _reasoning_guard = EnvGuard::set("OCTOCLAW_CODEX_REASONING_EFFORT", Some("medium"));
