...
//!
//! Wire protocol (OctoClaw serial JSON):
//! ```text
...
/// Default baud rate for OctoClaw serial devices.
...
/// Serial transport for OctoClaw hardware devices.
///
...
impl HardwareSerialTransport {
    ...
    /// Attempt a ping handshake to verify OctoClaw firmware is running.
    ///
    /// Opens the port, sends `{"cmd":"ping","params":{}}`, waits up to
    /// `PING_TIMEOUT_MS` for a response with `data.firmware == "octoclaw"`.
    ///
    /// Returns `true` if a OctoClaw device responds, `false` otherwise.
    ...
    pub async fn ping_handshake(&self) -> bool {
        ...
        match result {
            Ok(Ok(resp)) => {
                // Accept if firmware field is "octoclaw" (in data or top-level)
                resp.ok
                    && resp
                        .data
                        .get("firmware")
