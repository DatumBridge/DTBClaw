//! Plugin manifest — the `octoclaw.plugin.toml` descriptor.
//!
//! Mirrors OpenClaw's `openclaw.plugin.json` but uses TOML to match
//! OctoClaw's existing config format.
...
const SUPPORTED_WIT_PACKAGES: [&str; 3] =
    ["octoclaw:hooks", "octoclaw:tools", "octoclaw:providers"];
...
pub const PLUGIN_MANIFEST_FILENAME: &str = "octoclaw.plugin.toml";
...
/// Load and parse `octoclaw.plugin.toml` from `root_dir`.
...
fn required_wit_package_for_capability(capability: &PluginCapability) -> &'static str {
    match capability {
        PluginCapability::Hooks | PluginCapability::ModifyToolResults => "octoclaw:hooks",
        PluginCapability::Tools => "octoclaw:tools",
        PluginCapability::Providers => "octoclaw:providers",
    }
}
...
pub fn validate_manifest_with_profile(
    ...
    if !manifest.tools.is_empty() && !declared_wit_packages.contains("octoclaw:tools") {
        anyhow::bail!("plugin tools require wit package 'octoclaw:tools@{SUPPORTED_WIT_MAJOR}.x'");
    }
    if !manifest.providers.is_empty() && !declared_wit_packages.contains("octoclaw:providers") {
        anyhow::bail!(
            "plugin providers require wit package 'octoclaw:providers@{SUPPORTED_WIT_MAJOR}.x'"
        );
    }
...
"#,
    ...
    fn manifest_requires_id_and_module_path_for_runtime_validation() {
        ...
        let valid = PluginManifest {
            ...
            wit_packages: vec!["octoclaw:hooks@1.0.0".into()],
            tools: vec![],
            providers: vec![],
        };
    ...
    fn manifest_rejects_unknown_wit_package() {
        let manifest = PluginManifest {
            ...
            capabilities: vec![],
            module_path: "plugins/demo.wasm".into(),
            wit_packages: vec!["octoclaw:unknown@1.0.0".into()],
            tools: vec![],
            providers: vec![],
        };
