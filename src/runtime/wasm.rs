...
//! The default OctoClaw binary excludes it to maintain the 4.6 MB size target.
...
impl RuntimeAdapter for WasmRuntime {
    ...
    fn storage_path(&self) -> PathBuf {
        self.workspace_dir
            .as_ref()
            .map_or_else(|| PathBuf::from(".octoclaw"), |w| w.join(".octoclaw"))
    }
...
mod tests {
    ...
    fn wasm_storage_path_default() {
        let rt = WasmRuntime::new(default_config());
        assert!(rt.storage_path().to_string_lossy().contains("octoclaw"));
    }
    ...
    fn wasm_storage_path_with_workspace() {
        ...
        assert_eq!(
            rt.storage_path(),
            PathBuf::from("/home/user/project/.octoclaw")
        );
    }
