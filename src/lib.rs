...
Examples:
  ...
  octoclaw channel add discord '{\"bot_token\":\"...\",\"name\":\"my-discord\"}'")]
    Add {
...
Examples:
  ...
  octoclaw channel bind-telegram 123456789")]
    BindTelegram {
...
pub enum MigrateCommands {
    /// Import OpenClaw data into this OctoClaw workspace (memory, config, agents)
    Openclaw {
...
Examples:
  ...
  octoclaw cron add-at 2025-12-31T23:59:00Z 'Happy New Year!'")]
    AddAt {
...
Examples:
  octoclaw cron add-every 60000 'Ping heartbeat'     # every minute
  octoclaw cron add-every 3600000 'Hourly report'    # every hour")]
    AddEvery {
...
Examples:
  ...
  octoclaw cron once 2h 'Follow up on deployment'
  octoclaw cron once 1d 'Daily check'")]
    Once {
        /// Delay duration
        delay: String,
        /// Command to run
...
Examples:
  octoclaw cron update <task-id> --expression '0 8 * * *'
  octoclaw cron update <task-id> --tz Europe/London --name 'Morning check'
  octoclaw cron update <task-id> --command 'Updated message'")]
    Update {
        /// Task ID
        id: String,
        /// New cron expression
...
Examples:
  ...
  octoclaw hardware introspect COM3")]
    Introspect {
...
Examples:
  ...
  octoclaw peripheral add esp32 /dev/ttyUSB0")]
    Add {
    ...
    /// Flash OctoClaw firmware to Arduino (creates .ino, installs arduino-cli if needed, uploads)
    #[command(long_about = "\
Flash OctoClaw firmware to an Arduino board.

Generates the .ino sketch, installs arduino-cli if it is not \
already available, compiles, and uploads the firmware.

Examples:
  octoclaw peripheral flash
  octoclaw peripheral flash --port /dev/cu.usbmodem12345
  octoclaw peripheral flash -p COM3")]
    Flash {
    ...
    /// Flash OctoClaw firmware to Nucleo-F401RE (builds + probe-rs run)
    FlashNucleo,
}
