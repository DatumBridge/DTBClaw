...
/// and forwards PRIVMSG messages to the `OctoClaw` message bus.
...
impl Channel for IrcChannel {
    ...
    async fn listen(&self, tx: mpsc::Sender<ChannelMessage>) -> anyhow::Result<()> {
        ...
        Self::send_raw(
            &mut writer,
            &format!("USER {} 0 * :OctoClaw", self.username),
        )
        .await?;
...
mod tests {
    ...
    fn new_stores_all_fields() {
        ...
        assert_eq!(ch.username, "octoclaw");
    ...
    fn irc_config_serde_roundtrip() {
        ...
        assert_eq!(parsed.username.as_deref(), Some("octoclaw"));
