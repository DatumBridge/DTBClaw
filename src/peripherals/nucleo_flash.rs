//! Flash OctoClaw Nucleo-F401RE firmware via probe-rs.
//!
...
/// Flash OctoClaw Nucleo firmware. Builds from firmware/octoclaw-nucleo.
pub fn flash_nucleo_firmware() -> Result<()> {
    ...
    // CARGO_MANIFEST_DIR = repo root (octoclaw's Cargo.toml)
    let repo_root = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    let firmware_dir = repo_root.join("firmware").join("octoclaw-nucleo");
    if !firmware_dir.join("Cargo.toml").exists() {
        anyhow::bail!(
            "Nucleo firmware not found at {}. Run from octoclaw repo root.",
            firmware_dir.display()
        );
    }

    println!("Building OctoClaw Nucleo firmware...");
    ...
    println!("OctoClaw Nucleo firmware flashed successfully.");
