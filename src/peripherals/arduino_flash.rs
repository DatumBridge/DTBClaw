//! Flash OctoClaw Arduino firmware via arduino-cli.
//!
...
/// OctoClaw Arduino Uno base firmware (capabilities, gpio_read, gpio_write).
const FIRMWARE_INO: &str = include_str!("../../firmware/octoclaw-arduino/octoclaw-arduino.ino");

const FQBN: &str = "arduino:avr:uno";
const SKETCH_NAME: &str = "octoclaw-arduino";
...
/// Flash OctoClaw firmware to Arduino at the given port.
pub fn flash_arduino_firmware(port: &str) -> Result<()> {
    ensure_arduino_cli()?;
    ensure_avr_core()?;

    let temp_dir = std::env::temp_dir().join(format!("octoclaw_flash_{}", uuid::Uuid::new_v4()));
    ...
    // Compile
    println!("Compiling OctoClaw Arduino firmware...");
    ...
    println!("OctoClaw firmware flashed successfully.");
