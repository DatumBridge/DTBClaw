...
pub async fn run_wizard_with_migration(
    ...
    println!(
        "  {}",
        style("Welcome to OctoClaw — the fastest, smallest AI assistant.")
            .white()
            .bold()
    );
    println!(
    ...
    print_step(3, 11, "Channels (How You Talk to OctoClaw)");
    ...
    if has_channels && config.api_key.is_some() {
        ...
        if launch {
            ...
            std::env::set_var("OCTOCLAW_AUTOSTART_CHANNELS", "1");
        }
    }
...
pub async fn run_channels_repair_wizard() -> Result<Config> {
    ...
    print_step(1, 1, "Channels (How You Talk to OctoClaw)");
...
/// Use `octoclaw onboard` or `octoclaw onboard --api-key sk-... --provider openrouter --memory sqlite|lucid|cortex-mem`.
/// Use `octoclaw onboard --interactive` for the full wizard.
...
async fn maybe_run_openclaw_migration(
    ...
    let should_run = if options.enabled {
        ...
        Confirm::new()
            .with_prompt(
                "  Merge OpenClaw data into this OctoClaw workspace now? (preserve existing data)",
            )
            .default(true)
            .interact()?
    } else {
...
fn resolve_quick_setup_dirs_with_home(home: &Path) -> (PathBuf, PathBuf) {
    ...
    let config_dir = home.join(".octoclaw");
...
async fn run_quick_setup_with_home(
    ...
    let (octoclaw_dir, workspace_dir) = resolve_quick_setup_dirs_with_home(home);
    ...
    if credential_override.is_none() {
        if provider_supports_keyless_local_usage(&provider_name) {
            println!("    1. Chat:     octoclaw agent -m \"Hello!\"");
            println!("    2. Gateway:  octoclaw gateway");
            println!("    3. Status:   octoclaw status");
            ...
            if canonical_provider_name(&provider_name) == "copilot" {
                ...
                println!(
                    "    1. Login:             octoclaw auth login --provider {}",
                    provider_name
                );
                println!("    2. Chat:              octoclaw agent -m \"Hello!\"");
                println!("    3. Gateway:           octoclaw gateway");
                println!("    4. Status:            octoclaw status");
            }
        } else {
            ...
            println!("    2. Or edit:           ~/.octoclaw/config.toml");
            println!("    3. Chat:              octoclaw agent -m \"Hello!\"");
            println!("    4. Gateway:           octoclaw gateway");
        }
    } else {
        println!("    1. Chat:     octoclaw agent -m \"Hello!\"");
        println!("    2. Gateway:  octoclaw gateway");
...
pub async fn run_models_refresh(
    ...
    if !force {
        ...
        .await?
        {
            ...
            println!();
            println!(
                "Tip: run `octoclaw models refresh --force --provider {}` to fetch latest now.",
                provider_name
            );
            return Ok(());
        }
...
pub async fn run_models_list(config: &Config, provider_override: Option<&str>) -> Result<()> {
    ...
    let Some(cached) = cached else {
        println!();
        println!(
            "  No cached models for '{provider_name}'. Run: octoclaw models refresh --provider {provider_name}"
        );
        println!();
        return Ok(());
    };
...
async fn setup_provider(workspace_dir: &Path) -> Result<(String, String, String, Option<String>)> {
    ...
    if providers.is_empty() {
        ...
        print_bullet("OctoClaw works with ANY API that speaks the OpenAI chat completions format.");
    ...
    let api_key = if provider_name == "ollama" {
        ...
        if key.trim().is_empty() {
            print_bullet(
                "No token provided. OctoClaw will open the GitHub device login flow on first use.",
            );
        }
        ...
        if crate::providers::gemini::GeminiProvider::has_cli_credentials() {
            ...
            print_bullet("OctoClaw will reuse your existing Gemini CLI authentication.");
            println!();
...
fn setup_tool_mode() -> Result<(ComposioConfig, SecretsConfig)> {
    print_bullet("Choose how OctoClaw connects to external apps.");
    ...
    let composio_config = if choice == 1 {
        ...
        print_bullet("OctoClaw uses Composio as a tool — your core agent stays local.");
        println!();
    ...
    // ── Encrypted secrets ──
    println!();
    print_bullet("OctoClaw can encrypt API keys stored in config.toml.");
...
fn setup_hardware() -> Result<HardwareConfig> {
    print_bullet("OctoClaw can talk to physical hardware (LEDs, sensors, motors).");
    ...
    let choice = Select::new()
        .with_prompt("  How should OctoClaw interact with the physical world?")
        .items(&options)
        .default(recommended)
        .interact()?;
...
fn setup_memory() -> Result<MemoryConfig> {
    print_bullet("Choose how OctoClaw stores and searches memories.");
    print_bullet("You can always change this later in config.toml.");
    println!();
...
fn setup_identity_backend() -> Result<IdentityConfig> {
    print_bullet("Choose the identity format OctoClaw should scaffold for this workspace.");
...
fn setup_channels() -> Result<ChannelsConfig> {
    print_bullet("Channels let you talk to OctoClaw from anywhere.");
        ...
        match choice {
            ChannelMenuChoice::Telegram => {
                ...
                println!(
                    ...
                    style("— talk to OctoClaw from Telegram").dim()
                );
            ...
            ChannelMenuChoice::Discord => {
                ...
                println!(
                    ...
                    style("— talk to OctoClaw from Discord").dim()
                );
            ...
            ChannelMenuChoice::Slack => {
                ...
                println!(
                    ...
                    style("— talk to OctoClaw from Slack").dim()
                );
            ...
            ChannelMenuChoice::IMessage => {
                ...
                print_bullet("OctoClaw reads your iMessage database and replies via AppleScript.");
                print_bullet(
            ...
            ChannelMenuChoice::WhatsApp => {
                ...
                if mode_idx == 0 {
                    ...
                    let session_path: String = Input::new()
                        ...
                        .default("~/.octoclaw/state/whatsapp-web/session.db".into())
                        .interact_text()?;
                ...
                let verify_token: String = Input::new()
                    ...
                    .default("octoclaw-whatsapp-verify".into())
                    .interact_text()?;
                ...
                config.whatsapp = Some(WhatsAppConfig {
                    ...
                    app_secret: None, // Can be set via OCTOCLAW_WHATSAPP_APP_SECRET env var
                    session_path: None,
                    pair_phone: None,
                    pair_code: None,
                    allowed_numbers,
            ...
            ChannelMenuChoice::LarkFeishu => {
                ...
                println!(
                    ...
                    style("— talk to OctoClaw from Lark or Feishu").dim()
                );
                print_bullet(
            ...
            ChannelMenuChoice::Nostr => {
                ...
                print_bullet("OctoClaw will listen for encrypted DMs on Nostr relays.");
...
fn print_summary(config: &Config) {
    ...
    println!(
        "  {}  {}",
        style("⚡").cyan(),
        style("OctoClaw is ready!").white().bold()
    );
    println!(
        "  {}",
        style("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━").cyan()
    ...
    if config.api_key.is_none() && !provider_supports_keyless_local_usage(provider) {
        if canonical_provider == "copilot" {
            ...
            println!("       {}", style("octoclaw agent -m \"Hello!\"").yellow());
            println!(
                "       {}",
            ...
            println!(
                "       {}",
                style("octoclaw auth login --provider openai-codex --device-code").yellow()
            );
            ...
            println!(
                "       {}",
                style(
                    "or: octoclaw auth paste-token --provider anthropic --auth-kind authorization"
                )
                .yellow()
            );
        } else {
    ...
    if has_channels {
        ...
        println!("       {}", style("octoclaw channel start").yellow());
        println!();
        step += 1;
    }
    ...
    println!(
        "       {}",
        style("octoclaw agent -m \"Hello, OctoClaw!\"").yellow()
    );
    println!();
    step += 1;
    ...
    println!("       {}", style("octoclaw agent").yellow());
    println!();
    step += 1;
    ...
    println!("       {}", style("octoclaw status").yellow());
...
mod tests {
    ...
    async fn run_quick_setup_with_clean_env(
        ...
        let _workspace_env = EnvVarGuard::unset("OCTOCLAW_WORKSPACE");
        let _config_env = EnvVarGuard::unset("OCTOCLAW_CONFIG_DIR");
    ...
    async fn quick_setup_existing_config_requires_force_when_non_interactive() {
        let tmp = TempDir::new().unwrap();
        let octoclaw_dir = tmp.path().join(".octoclaw");
        let config_path = octoclaw_dir.join("config.toml");

        tokio::fs::create_dir_all(&octoclaw_dir).await.unwrap();
    ...
    async fn quick_setup_respects_zero_claw_workspace_env_layout() {
        let _env_guard = env_lock().lock().await;
        let tmp = TempDir::new().unwrap();
        let workspace_root = tmp.path().join("octoclaw-data");
        let workspace_dir = workspace_root.join("workspace");
        let expected_config_path = workspace_root.join(".octoclaw").join("config.toml");
        ...
        .await
        .expect("quick setup should honor OCTOCLAW_WORKSPACE");
    ...
    async fn scaffold_uses_defaults_for_empty_context() {
        ...
        let identity = tokio::fs::read_to_string(tmp.path().join("IDENTITY.md"))
            ...
            .unwrap();
        assert!(
            identity.contains("**Name:** OctoClaw"),
            "should default agent name to OctoClaw"
        );
    ...
    async fn scaffold_handles_special_characters_in_names() {
        ...
        let soul = tokio::fs::read_to_string(tmp.path().join("SOUL.md"))
            .await
            .unwrap();
        assert!(soul.contains("OctoClaw-v2"));
    }
    ...
    fn memory_config_defaults_for_hybrid_enable_sqlite_hygiene() {
        ...
        assert_eq!(config.purge_after_days, 30);
        assert_eq!(config.embedding_cache_size, 10000);
        assert_eq!(config.qdrant.collection, "octoclaw_memories");
    }
