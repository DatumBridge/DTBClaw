...
A complete toolkit for building AI-powered robots with OctoClaw. Designed for Raspberry Pi deployment with offline Ollama inference.
...
### 1. Build OctoClaw with robot tools

```bash
# Clone and build
git clone https://github.com/octoclaw-labs/octoclaw
cd octoclaw
cargo build -p octoclaw-robot-kit --release
```
...
# Copy config
mkdir -p ~/.octoclaw
cp crates/robot-kit/robot.toml ~/.octoclaw/
cp crates/robot-kit/SOUL.md ~/.octoclaw/workspace/
...
# Test in mock mode
./target/release/octoclaw agent -m "Say hello and show a happy face"

# Test with real hardware
# (after configuring robot.toml)
./target/release/octoclaw agent -m "Move forward 1 meter"
```
...
```rust
use octoclaw_robot_kit::{create_tools, RobotConfig};
...
If you want runtime registration in `octoclaw`, add a thin adapter that maps this
...
mkdir octoclaw-robot-kit
cp -r target/release/octoclaw octoclaw-robot-kit/
cp -r examples/robot_kit octoclaw-robot-kit/
cp -r ~/.octoclaw octoclaw-robot-kit/dot-octoclaw

# Include models
mkdir -p octoclaw-robot-kit/models
cp ~/.octoclaw/models/ggml-base.bin octoclaw-robot-kit/models/
# Note: Ollama models are large, may want to download on target

# Create tarball
tar -czvf octoclaw-robot-kit.tar.gz octoclaw-robot-kit/

# Copy to USB
cp octoclaw-robot-kit.tar.gz /media/usb/TarBalls/
```
