...
Complete guide to setting up a OctoClaw-powered robot on Raspberry Pi 5.
...
mkdir -p ~/.octoclaw/models
cp models/ggml-base.bin ~/.octoclaw/models/
```
...
mkdir -p ~/.octoclaw/models/piper
cd ~/.octoclaw/models/piper
wget https://huggingface.co/rhasspy/piper-voices/resolve/main/en/en_US/lessac/medium/en_US-lessac-medium.onnx
wget https://huggingface.co/rhasspy/piper-voices/resolve/main/en/en_US/lessac/medium/en_US-lessac-medium.onnx.json

# Test
echo "Hello, I am your robot!" | piper --model ~/.octoclaw/models/piper/en_US-lessac-medium.onnx --output_file test.wav
aplay test.wav
```
...
### 7. Build OctoClaw Robot Kit

```bash
# Clone repo (or copy from USB)
git clone https://github.com/theonlyhennygod/octoclaw
cd octoclaw

# Build robot kit
cargo build --release -p octoclaw-robot-kit

# Build main octoclaw (optional, if using as agent)
cargo build --release
```
...
```bash
mkdir -p ~/.octoclaw
nano ~/.octoclaw/robot.toml
```

```toml
# ~/.octoclaw/robot.toml - Real Hardware Configuration
...
# Test speaker
echo "Testing speaker" | piper --model ~/.octoclaw/models/piper/en_US-lessac-medium.onnx --output_file - | aplay -D plughw:0,0
...
FIFO_PATH = "/tmp/octoclaw_sensors.fifo"
...
### Start OctoClaw Agent

```bash
# Configure OctoClaw to use robot tools
cat > ~/.octoclaw/config.toml << 'EOF'
...
cp ~/octoclaw/crates/robot-kit/SOUL.md ~/.octoclaw/workspace/

# Start agent
./target/release/octoclaw agent
```
...
# Start sensor loop
if [ ! -p /tmp/octoclaw_sensors.fifo ]; then
    mkfifo /tmp/octoclaw_sensors.fifo
fi
...
# Start octoclaw
cd ~/octoclaw
./target/release/octoclaw daemon &
AGENT_PID=$!
...
```bash
# /etc/systemd/system/octoclaw-robot.service
sudo tee /etc/systemd/system/octoclaw-robot.service << 'EOF'
[Unit]
Description=OctoClaw Robot
After=network.target ollama.service

[Service]
Type=simple
User=pi
WorkingDirectory=/home/pi/octoclaw
...
sudo systemctl enable octoclaw-robot
sudo systemctl start octoclaw-robot

# Check status
sudo systemctl status octoclaw-robot
journalctl -u octoclaw-robot -f  # View logs
```
