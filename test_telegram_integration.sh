#!/bin/bash
# OctoClaw Telegram Integration Test Suite
...
if [ -f "target/release/octoclaw" ]; then
    BINARY_SIZE=$(ls -lh target/release/octoclaw | awk '{print $5}')
    SIZE_BYTES=$(stat -f%z target/release/octoclaw 2>/dev/null || stat -c%s target/release/octoclaw)
...
CONFIG_PATH="$HOME/.octoclaw/config.toml"
if [ -f "$CONFIG_PATH" ]; then
    ...
    if grep -q "\[channels_config.telegram\]" "$CONFIG_PATH"; then
        ...
        warn "Telegram not configured - run 'octoclaw onboard' first"
    fi
else
    warn "No config file found - run 'octoclaw onboard' first"
fi
...
START_TIME=$(date +%s)
HEALTH_OUTPUT=$(timeout 10 target/release/octoclaw channel doctor 2>&1 || true)
END_TIME=$(date +%s)
